class DoneDoneApi
  ...
  def self.conn
    connection = Faraday.new(:url => 'https://tytan.mydonedone.com/issuetracker/api/v2')
    connection.basic_auth('Marowe', 'polak033')
    connection
  end
  ...
  def self.export_issues(local_issue_id)
    local_issue = LocalIssue.find(local_issue_id)
    export_params =
    {
      title: local_issue.title,
      description: local_issue.desription,
      priority_level_id: local_issue.priority_level_id,
      fixer_id: local_issue.fixer_id,
      tester_id: local_issue.tester_id
    }

    conn.post '/issuetracker/api/v2/projects/59791/issues.json', export_params
  end
